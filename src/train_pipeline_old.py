...
def train_pipeline(env, config, eval_qnet, seedvec=None):
    ...
    for i_episode in range(config.sample_num_traj):  # 評価trajectoryの作成
        ...
        while not done:
            ...
            action = epsilon_greedy_action(state, eval_qnet, config.behavior_epsilon, config.action_size,
            ...
            p_pie = epsilon_greedy_action_prob(state, eval_qnet, 0, config.action_size,
            ...
            for memory_idx in fold_idx_list_k:
                memory_k[memory_idx].push(state_re, action, next_state_re, reward, done, isweight, acc_isweight,
                                          n_steps, factual,
                                          last_factual, acc_soft_isweight, soft_isweight, soft_pie, p_pie, p_pib)

            # dev_foldのmemoryにログを残す
            dev_memory_k[dev_fold].push(state_re, action, next_state_re, reward, done, isweight, acc_isweight, n_steps,
                                        factual,
                                        last_factual, acc_soft_isweight, soft_isweight, soft_pie, p_pie, p_pib)
    ...
    dml_dr_cross_k_estpz = np.zeros(config.sample_num_traj)
    dml_dr_cross_k_estpz_wis = np.zeros(config.sample_num_traj)
    ...
    for fold_idx in range(fold_num):
        ...
        fold_dev_memory = dev_memory_k[dev_fold]
        fold_memory.flatten()
        ...
        dml_dr_cross_k_estpz[fold_indicator == fold_idx] = dml_dr_cross_estpz_k_fold[fold_indicator == fold_idx]
        ...
        dml_dr_cross_k_estpz_wis[fold_indicator == fold_idx] = dml_dr_cross_estpz_k_fold_wis[fold_indicator == fold_idx]
    ...
    print(
        '| Sampling: {:.3f}s | Preprocess for MRDR: {:.3f}s | Learn MRDR: {:.3f}s | Learn MDP: {:.3f}s | Learn TC: {:.3f}s '
    ...
    results = [mv, dr,
               ...
               dml_dr_cross_k_nd, dml_dr_cross_k_estpz_nd, dml_dr_cross_k_estpz_wis_nd,
               wdr, sdr, swdr, mv_bsl, dr_bsl, wdr_bsl, sdr_bsl, swdr_bsl,
