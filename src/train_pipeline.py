...
def compute_values(traj_set, model, is_done, policy_qnet, config, max_length , model_type='MDP', soften=False): #ここでpizeroの予測値を返す様にする
    ...
    for i_step in range(max_length):
        ...
        if model_type == 'MDP': # doubly robustとかが使う
            ...
            pie_actions = epsilon_greedy_action_batch(state_tensor[:, i_step, :], policy_qnet, 0, config.action_size) # evaluation policy actio(pure greedy)
...
def rollout_batch_pz(init_states, mdpnet, is_done, num_rollout, policy_qnet, epsilon, action_size, maxlength, config, init_done=None, init_actions=None):
    ori_batch_size = init_states.size()[0]
    state_dim = init_states.size()[1]
    batch_size = init_states.size()[0]*num_rollout
    init_states = init_states.repeat(num_rollout,1)
...
def compute_pizero(traj_set, model, is_done, policy_qnet, config, model_type='MDP', soften=False):
    ...
    for i_step in range(config.max_length):
        pizero_value[:, i_step] = rollout_batch_pz(state_tensor[:, i_step, :], model, is_done, config.eval_num_rollout,
                                                   ...
                                                   init_done=done_tensor[:, i_step], init_actions=action_tensor[:, i_step, :])
...
# 同じQ functionをつかって、
    ...
    for i_episode in range(config.sample_num_traj): # 評価trajectoryの作成
        ...
        while not done:
            ...
            for memory_idx in fold_idx_list_k:
                nd_memory_k[memory_idx].push(state_re, action, next_state_re, reward, done, isweight, acc_isweight, n_steps, factual,
                            last_factual, acc_soft_isweight, soft_isweight, soft_pie, p_pie, p_pib)
            # target fold data save
            nd_memory_test[fold].push(state_re, action, next_state_re, reward, done, isweight, acc_isweight, n_steps, factual,
                            last_factual, acc_soft_isweight, soft_isweight, soft_pie, p_pie, p_pib)
    ...
    for fold_idx in range(fold_num):
        ...
        # reward predictin
        mu_hat = reward_prediction(traj_set, mdpnet_dml_crossfit_k, max_length=config.max_length, config=config, soften=False)
